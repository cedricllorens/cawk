...
# -----------------------------------------------------------------------
# Parallel debug checker - runs gmake check_parallel and logs results
# Also creates phases_parallel.txt mapping with command descriptions
# -----------------------------------------------------------------------

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_FILE="$SCRIPT_DIR/check_parallel_debug.log"
PHASES_FILE="$SCRIPT_DIR/phases_parallel.txt"

echo "cawk running full cawk check parallel (faster) ----"
echo "cawk executes gmake check with MAKE_PARALLEL=yes ----"
echo ""
echo "This will execute all phases in parallel and generate checkdiff.new"
echo "Command mapping: $PHASES_FILE"
echo "Detailed log:    $LOG_FILE"
echo ""

# Generate phases_parallel.txt dynamically from Makefile
...
{
    echo "cawk check parallel debug started ----"
    echo "Timestamp: $(date)"
    echo ""
    gmake check_parallel
    echo ""
    echo "cawk check parallel debug completed ----"
    echo "Timestamp: $(date)"
} | tee "$LOG_FILE"
