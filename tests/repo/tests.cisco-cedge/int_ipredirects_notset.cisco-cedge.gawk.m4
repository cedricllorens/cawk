...
#
# @test_name : int_ipredirects_notset.cisco-cedge.gawk
...
`/^!$/', `
	if ( noipredirects == 1 ) {
		print_err("int_ipredirects_notset.cisco-cedge.gawk",interface" <ip redirects> is set",interfaceno,"high","error");
		pass = 0;
	}
', 
...
ENDFILE {
	if ( pass ) print_err("int_ipredirects_notset.cisco-cedge.gawk","interface <ip redirects> is not set",0,"high","pass");
}
