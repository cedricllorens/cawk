...
#
# @test_name : int_ipunreachables_notset.cisco-xe.gawk
# @supplier : cisco-xe
...
`/^!$/', `
	if ( unreachable == 1 ) {
		print_err("int_ipunreachables_notset.cisco-xe.gawk",interface" <ip unreachables> is set",interfaceno,"high","error");
		pass = 0;
	}
', 
...
ENDFILE {
	if ( pass ) print_err("int_ipunreachables_notset.cisco-xe.gawk","interface <ip unreachables> is not set",0,"high","pass");
}
