...
#
# @test_name : int_cdpenable_notset.cisco-xe.gawk
# @supplier : cisco-xe
...
`/^!$/', `
	if ( cdpenable == 1 ) {
		print_err("int_cdpenable_notset.cisco-xe.gawk",interface" <cdp enable> is set",interfaceno,"high","error");
		pass = 0;
	}
', 
...
ENDFILE {
	if ( pass ) print_err("int_cdpenable_notset.cisco-xe.gawk","interface <cdp enable> is not set",0,"high","pass");
}
