...
#
# @test_name : int_directedbroadcast_notset.nokia-sros.gawk
# @supplier : nokia-sros
...
`/^%SED_BK_NOKIA%{3}ip directed-broadcast/',`
	print_err("int_directedbroadcast_notset.nokia-sros.gawk",rm_blank_before(interface)" "rm_blank_before($0),FNR,"high","error");
	pass = 0;
',
`',`'
...
ENDFILE {
	if ( pass ) print_err("int_directedbroadcast_notset.nokia-sros.gawk","interface <ip directed-broadcast> is not set",0,"high","pass");
}
