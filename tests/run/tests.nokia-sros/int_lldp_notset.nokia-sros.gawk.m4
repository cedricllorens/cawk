...
#
# @test_name : int_lldp_notset.nokia-sros.gawk
# @supplier : nokia-sros
# @purpose : check if an interface has no <lldp>
...
`/^%SED_BK_NOKIA%{3}lldp/',`
	print_err("int_lldp_notset.nokia-sros.gawk",rm_blank_before(interface)" "rm_blank_before($0),FNR,"high","error");
	pass = 0;
',
`',`'
...
ENDFILE {
	if ( pass ) print_err("int_lldp_notset.nokia-sros.gawk","interface <lldp> is not set",0,"high","pass");
}
