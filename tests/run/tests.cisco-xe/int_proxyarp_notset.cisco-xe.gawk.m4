...
#
# @test_name : int_proxyarp_notset.cisco-xe.gawk
# @supplier : cisco-xe
...
`/^!$/', `
	if ( proxyarp == 1 ) {
		print_err("int_proxyarp_notset.cisco-xe.gawk",interface" <ip proxy-arp> is set",interfaceno,"high","error");
		pass = 0;
	}
', 
...
ENDFILE {
	if ( pass ) print_err("int_proxyarp_notset.cisco-xe.gawk","interface <ip proxy-arp> is not set",0,"high","pass");
}
